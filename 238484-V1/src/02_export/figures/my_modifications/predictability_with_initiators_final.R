...
regr_data_inter_target_avg <- regr_data_inter_target %>%
  mutate(
    gdp_growth_avg = rowMeans(select(., gdp_growth_lag1, gdp_growth_lag2, gdp_growth_lag3, gdp_growth_lag4), na.rm = TRUE),
    inflation_avg  = rowMeans(select(., inflation_lag1, inflation_lag2, inflation_lag3, inflation_lag4), na.rm = TRUE),
    openness_avg   = rowMeans(select(., openness_lag1, openness_lag2, openness_lag3, openness_lag4), na.rm = TRUE),
...
inter_target_simple_countryfe <- fixest::feglm(
  data = regr_data_inter_target,
  fixef = c("iso3"),
  panel.id = ~ iso3 + year,
  family = binomial(link = "logit"),
...
summary(inter_target_simple_countryfe)

# average
inter_target_avg_countryfe <- fixest::feglm(
  data = regr_data_inter_target_avg,
  fixef = c("iso3"),
  panel.id = ~ iso3 + year,
  family = binomial(link = "logit"),
  vcov = ~ iso3,
...
summary(inter_target_avg_countryfe)
...
inter_target_conditional_full <- fixest::feglm(
  data = regr_data_inter_conditional,
  #fixef = c("iso3"),
  panel.id = ~ iso3 + year,
  family = binomial(link = "logit"),
...
summary(inter_target_conditional_full)
...
pacman::p_load(modelsummary, flextable, officereer)


models <- list(
  "Initiator w. Average Lags" = inter_starter_avg,
  "Initiator w. Lag 1" = inter_starter_simple,
  "Initiator w. Full Lags" = inter_starter_full_feiso,
  "Non-initiator w. Average Lags" = inter_target_avg_countryfe,
  "Non-initiator w. Lag 1" = inter_target_simple_countryfe,
  "Non-initiator w. Full Lags" = inter_target_full_countryfe,
  "Non-initiator Conditional w. Average Lags" = inter_target_conditional_avg,
  "Non-initiator Conditional w. Lag1" = inter_target_conditional_countryfe,
  "Non-initiator Conditional w. Full Lags" = inter_target_conditional_full
)
...
cm <- c(
  ...
  "gdp_growth_lag2"    = "GDP Growth (t-2)",
  "gdp_growth_lag3"    = "GDP Growth (t-3)",
  "gdp_growth_lag4"    = "GDP Growth (t-4)",
  "gdp_growth_avg"     = "GDP Growth (Avg. Lags)",
  
  # Inflation
  "inflation_lag1"     = "Inflation (t-1)",
  "inflation_lag2"     = "Inflation (t-2)",
  "inflation_lag3"     = "Inflation (t-3)",
  "inflation_lag4"     = "Inflation (t-4)",
  "inflation_avg"      = "Inflation (Avg. Lags)",
  
  # Openness
  "openness_lag1"      = "Trade Openness (t-1)",
  "openness_lag2"      = "Trade Openness (t-2)",
  "openness_lag3"      = "Trade Openness (t-3)",
  "openness_lag4"      = "Trade Openness (t-4)",
  "openness_avg"       = "Trade Openness (Avg. Lags)",
  
  # Military Expenditures
  "milex_gdp_lag1"     = "Military Exp/GDP (t-1)",
  "milex_gdp_lag2"     = "Military Exp/GDP (t-2)",
  "milex_gdp_lag3"     = "Military Exp/GDP (t-3)",
  "milex_gdp_lag4"     = "Military Exp/GDP (t-4)",
  "milex_gdp_avg"      = "Military Exp/GDP (Avg. Lags)",
  
  # Peace Years & Polynomials (for Temporal Dependence)
  "peace_years"        = "Peace Years",
  "peace_years_sq"     = "Peace Years²",
  "peace_years_cub"    = "Peace Years³",
  
  # Other Controls
  "borders_lag1"       = "Borders (t-1)",
  "cowmaj_lag1"        = "Major Power (t-1)",
...
modelsummary(
  ...
  gof_omit = "AIC|Log.Lik.",   # Customize which GOF stats to keep
  output = "src/02_export/figures/my_modifications/results/regression_table.docx"
)
...
modelsummary(
  ...
  stars = TRUE,               # Adds significance stars (e.g., *** p < 0.01)
  gof_omit = "AIC|Log.Lik.",   # Customize which GOF stats to keep
  output = "src/02_export/figures/my_modifications/results/target_conditional_regression_table.docx"
)
modelsummary(
  target_models,
  coef_map = cm,
  stars = TRUE,               # Adds significance stars (e.g., *** p < 0.01)
  gof_omit = "AIC|Log.Lik.",   # Customize which GOF stats to keep
  output = "src/02_export/figures/my_modifications/results/target_regression_table.docx"
)
modelsummary(
  init_models,
  coef_map = cm,
  stars = TRUE,               # Adds significance stars (e.g., *** p < 0.01)
  gof_omit = "AIC|Log.Lik.",   # Customize which GOF stats to keep
  output = "src/02_export/figures/my_modifications/results/initiator_regression_table.docx"
)
